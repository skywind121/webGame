 ...
 *
 * 専用制御文字
 * \V[n,m,space or zero](m桁分の空白orゼロ埋めした変数の値)
 *  ※ space = 空白埋め   zero = ゼロ埋め
 ...
 * \fs[n] フォントサイズを n に指定する、0 を指定すると標準サイズに戻る
 *
 * 利用規約：
...
(function() {
  ...
  var getUsingVariables = function(text) {
    ...
    text = text.replace(/\x1bV\[(\d+)\,\s*(\d+),\s*(\w+)\]/gi, function() {
  ...
  Window_Base.prototype.convertEscapeCharacters = function(text) {
    ...
    text = text.replace(/\x1bV\[(\d+)\,\s*(\d+),\s*(\w+)\]/gi, function() {
      return this.getVariablePadZero($gameVariables.value(parseInt(arguments[1], 10)), arguments[2], arguments[3]);
    }.bind(this));
  ...
  Window_Base.prototype.getVariablePadZero = function(value, digit, space) {
    if (space == "space") {
      var number = (value + "").padStart(digit)
      return number
    }
  ...
  Sprite_Picture.prototype._processText = function(bitmap) {
    ...
    var textHeight = this.hiddenWindow.calcTextHeight(textState, false);
    var dTextheight = this.hiddenWindow.calcDTextHeight(textState, false);
    textState.height = textHeight > dTextheight ? textHeight : dTextheight;
    textState.index = 0;
    ...
    if (textState.text[textState.index] === '\x1b') {
      ...
      switch (code) {
        ...
        case 'OW':
          ...
          break;
        case 'FS':
          this.hiddenWindow.makeFontSize(this.dTextInfo, this.hiddenWindow.obtainEscapeParam(textState));
          break;
  ...
  Window_Hidden.prototype.makeFontSize = function(dTextInfo, fontSize){
    if((fontSize > 108 || fontSize < 12) && fontSize != 0) return;
    this.contents.fontSize = (fontSize == 0) ? dTextInfo.size || this.standardFontSize() : fontSize || this.contents.fontSize;
  }

  Window_Hidden.prototype.calcDTextHeight = function(textState, all) {
    ...
    for(var i = 0; i < maxLines; i++){
      ...
      var fs = lines[i].match(/\x1bfs\[(\d+)\]/g);
      if (fs) {
        for(var k = 0; k < fs.length; k++){
          var fontSize = (/(\d+)/.exec(fs[k]))[0];
          this.contents.fontSize = Number(fontSize) || this.contents.fontSize;
